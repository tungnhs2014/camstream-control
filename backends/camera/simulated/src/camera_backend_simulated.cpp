...
camstream_camera_status_t set_error(SimulatedCamera* camera, camstream_camera_status_t status,
                                    const char* message) noexcept {
    if (camera != nullptr) {
        try {
            camera->last_error = message != nullptr ? message : "unknown error";
        } catch (...) {
            LOGE("Simulated backend could not retain diagnostic text");
        }
    }
    return status;
}
...
bool configuration_is_supported(const camstream_camera_stream_config_v1& configuration) noexcept {
    ...
    const std::size_t source_length = bounded_string_length(configuration.source_identifier,
                                                            CAMSTREAM_CAMERA_MAX_SOURCE_ID_SIZE);
    ...
    *instance = nullptr;
    try {
        ...
        *instance = reinterpret_cast<camstream_camera_instance*>(camera);
        ...
        LOGE("Simulated backend instance allocation failed");
        return CAMSTREAM_CAMERA_STATUS_RESOURCE_ERROR;
    } catch (...) {
        LOGE("Unexpected exception while creating a simulated backend instance");
...
static camstream_camera_status_t simulated_open(camstream_camera_instance* instance, const char* source_identifier) {
    ...
    clear_error(camera);
    LOGI("Simulated camera opened: " << source_identifier);
    return CAMSTREAM_CAMERA_STATUS_OK;
}
...
static camstream_camera_status_t simulated_close(camstream_camera_instance* instance) {
    ...
    if (has_outstanding_frame(*camera)) {
        return set_error(camera, CAMSTREAM_CAMERA_STATUS_INVALID_STATE, "close requires every frame to be released");
    }

    camera->state = SimulatedState::Created;
    clear_error(camera);
    LOGI("Simulated camera closed");
    return CAMSTREAM_CAMERA_STATUS_OK;
}
...
static camstream_camera_status_t simulated_get_stream_configuration(camstream_camera_instance* instance,
                                                                    std::uint32_t index,
    ...
    if (camera->state != SimulatedState::Open) {
        return set_error(camera, CAMSTREAM_CAMERA_STATUS_INVALID_STATE, "get_stream_configuration requires Open state");
    }
    if (index != 0U) {
        return set_error(camera, CAMSTREAM_CAMERA_STATUS_INVALID_ARGUMENT,
...
static camstream_camera_status_t simulated_configure(camstream_camera_instance* instance,
    ...
    if (!configuration_is_supported(*requested)) {
        return set_error(camera, CAMSTREAM_CAMERA_STATUS_NOT_SUPPORTED,
                         "simulated backend supports only simulated0 64x48 YUYV at 30/1");
    }

    fill_supported_configuration(*active);
    camera->state = SimulatedState::Configured;
    clear_error(camera);
    LOGI("Simulated camera configured: 64x48 YUYV at 30/1");
    return CAMSTREAM_CAMERA_STATUS_OK;
}
...
static camstream_camera_status_t simulated_acquire_frame(camstream_camera_instance* instance,
    ...
    if (buffer == nullptr) {
        return set_error(camera, CAMSTREAM_CAMERA_STATUS_RESOURCE_ERROR, "all simulated frame buffers are outstanding");
    }

    const std::uint64_t sequence = camera->next_sequence++;
    for (std::size_t index = 0U; index < buffer->storage.size(); ++index) {
        buffer->storage[index] = static_cast<std::uint8_t>((sequence + static_cast<std::uint64_t>(index)) &
                                                           UINT64_C(0xff));
    }
...
static camstream_camera_status_t simulated_stop(camstream_camera_instance* instance) {
    ...
    clear_error(camera);
    LOGI("Simulated camera stopped");
    return CAMSTREAM_CAMERA_STATUS_OK;
}

static camstream_camera_status_t simulated_get_last_error(camstream_camera_instance* instance, char* buffer,
...
__attribute__((constructor)) static void register_simulated_backend() noexcept {
    ...
    if (status != CAMSTREAM_CAMERA_STATUS_OK) {
        LOGE("Simulated backend registration failed with status " << status);
    }
}
